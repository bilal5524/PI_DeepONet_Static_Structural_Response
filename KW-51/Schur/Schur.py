...
mat_data = scipy.io.loadmat('./Output_UI_UN_less.mat')
Output_new_two = mat_data['Output_UI'].astype(np.float64)

s_all = Output_new_two.copy()
mat_data = scipy.io.loadmat('./Input_mesh_UI_UN_less.mat')
xy_train_testing = mat_data['Input_mesh_UI'].astype(np.float64)

for idx, fraction_train in enumerate([0.2]):
...
#    y_test[:, :, 4] = 100000 * y_test[:, :, 4]
    ...
    losshistory, train_state = model.train(iterations=6000, batch_size=batch_size)
    ...
    error_arrays = [error_s, error_s2, error_s3, error_s4, error_s5, error_s6]
    variable_names = ['error_U1', 'error_U2', 'error_U3', 'error_R1','error_R2','error_R3']
    ...
    print('mean of relative L2 error of R2: {:.2e}'.format(error_s5.mean()))
    print('std of relative L2 error of R2: {:.2e}'.format(error_s5.std()))
    ...
    plt.hist(error_s5.flatten(), bins=15, label='R2')
    plt.xlabel('Relative L2 Error')
    plt.ylabel('Occurrence')
    plt.title(f'Histogram of Relative L2 Error - DeepONet {Value}')
    plt.legend()
